...
public class Util {
    ...
    public static String toShortVnd(long number) {
        if (number < 1000) {
            // Dưới 1 nghìn, trả về nguyên
        ...
        if (num >= 1_000_000_000L) { // Tỷ (10^9)
            return df.format(num / 1_000_000_000.0) + "tỷ vnd";
        } else if (num >= 1_000_000L) { // Triệu (10^6)
            return df.format(num / 1_000_000.0) + "tr vnd";
        } else if (num >= 1_000L) { // Nghìn (10^3)
            return df.format(num / 1_000.0) + "k vnd";
        } else {
