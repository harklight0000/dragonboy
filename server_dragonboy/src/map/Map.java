...
public class Map implements Runnable {
    ...
    private void initZone(int nZone, int maxPlayer) {
        switch (this.type) {
            ...
            case ConstMap.MAP_MA_BU -> nZone = MajinBuuService.AVAILABLE;
            case ConstMap.MAP_MABU_14H -> nZone = MajinBuu14H.AVAILABLE;
            case ConstMap.MAP_DOANH_TRAI -> nZone = RedRibbonHQ.AVAILABLE;
            case ConstMap.MAP_BAN_DO_KHO_BAU -> nZone = TreasureUnderSea.AVAILABLE;
            case ConstMap.MAP_CON_DUONG_RAN_DOC -> nZone = SnakeWay.AVAILABLE;
            case ConstMap.MAP_KHI_GAS_HUY_DIET -> nZone = DestronGas.AVAILABLE;
        }
        ...
        for (int i = 0; i < nZone; i++) {
            ...
            this.zones.add(zone);
            switch (this.type) {
                case ConstMap.MAP_BLACK_BALL_WAR -> BlackBallWarService.gI().addMapBlackBallWar(i, zone);
                case ConstMap.MAP_DOANH_TRAI -> RedRibbonHQService.gI().addMapDoanhTrai(i, zone);
                case ConstMap.MAP_BAN_DO_KHO_BAU -> TreasureUnderSeaService.gI().addMapBanDoKhoBau(i, zone);
                case ConstMap.MAP_CON_DUONG_RAN_DOC -> SnakeWayService.gI().addMapConDuongRanDoc(i, zone);
                case ConstMap.MAP_KHI_GAS_HUY_DIET -> DestronGasService.gI().addMapKhiGasHuyDiet(i, zone);
                case ConstMap.MAP_TAY_KARIN -> SuperDivineWaterService.gI().addZone(zone);
                case ConstMap.MAP_MABU_14H -> MajinBuu14HService.gI().addMapMaBu2H(i, zone);
            }
        }
    }
    ...
    public void initMob(byte[] mobTemp, byte[] mobLevel, int[] mobHp, short[] mobX, short[] mobY) {
        for (int i = 0; i < mobTemp.length; i++) {
            ...
            Template.MobTemplate temp = GameData.getMobTemplateByTemp(mobTempId);
            if (temp != null) {
                Mob mob = new Mob();
                mob.id = i;
                ...
                for (Zone zone : this.zones) {
                    Mob mobZone;
                    switch (mob.tempId) {
                        case ConstMob.HIRUDEGARN -> mobZone = new Hirudegarn(mob);
                        case ConstMob.VUA_BACH_TUOC -> mobZone = new VuaBachTuoc(mob);
                        case ConstMob.ROBOT_BAO_VE -> mobZone = new RobotBaoVe(mob);
                        case ConstMob.GAU_TUONG_CUOP -> mobZone = new GauTuongCuop(mob);
                        case ConstMob.VOI_CHIN_NGA -> mobZone = new VoiChinNga(mob);
                        case ConstMob.GA_CHIN_CUA -> mobZone = new GaChinCua(mob);
                        case ConstMob.NGUA_CHIN_LMAO -> mobZone = new NguaChinLmao(mob);
                        case ConstMob.PIANO -> mobZone = new Piano(mob);
                        default -> mobZone = new Mob(mob);
                    }
    ...
    private void initItem() {
        for (Zone zone : zones) {
            ...
            switch (this.mapId) {
                case 21 -> itemMap = new ItemMap(zone, 74, 1, 633, 315, -1);
                case 22 -> itemMap = new ItemMap(zone, 74, 1, 56, 315, -1);
                case 23 -> itemMap = new ItemMap(zone, 74, 1, 633, 320, -1);
                case 42 -> itemMap = new ItemMap(zone, 78, 1, 70, 288, -1);
                case 43 -> itemMap = new ItemMap(zone, 78, 1, 70, 264, -1);
                case 44 -> itemMap = new ItemMap(zone, 78, 1, 70, 288, -1);
                case 85 -> //1 sao đen
    ...
    public void initBoss() {
        for (Zone zone : zones) {
            short bossId = -1;
            switch (this.mapId) {
                case 42, 43, 44 -> bossId = BossID.BROLY_BASE;
                case 111 -> bossId = BossID.TAU_PAY_PAY_DONG_NAM_KARIN;
                case 114 -> bossId = BossID.DRABURA;
                case 115 -> bossId = BossID.BUI_BUI;
                case 117 -> bossId = BossID.BUI_BUI_2;
                case 118 -> bossId = BossID.YA_CON;
                case 119 -> bossId = BossID.DRABURA_2;
                case 120 -> bossId = BossID.MABU_12H;
                case 127 -> bossId = BossID.MABU;
                case 128 -> bossId = BossID.SUPERBU;
                case 131 -> bossId = BossID.TAN_BINH_5;
                case 132 -> bossId = BossID.CHIEN_BINH_5;
                case 133 -> bossId = BossID.DOI_TRUONG_5;
            }
            if (bossId != -1) {
