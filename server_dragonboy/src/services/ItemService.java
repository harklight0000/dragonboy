...
public class ItemService {
    ...
    public short getItemIdByIcon(short IconID) {
        for (int i = 0; i < GameData.ITEM_TEMPLATES.size(); i++) {
            if (GameData.ITEM_TEMPLATES.get(i).iconID == IconID) {
                return GameData.ITEM_TEMPLATES.get(i).id;
            }
        }
        return -1;
    }
    ...
    public ItemOptionTemplate getItemOptionTemplate(int id) {
        return GameData.ITEM_OPTION_TEMPLATES.get(id);
    }

    public Template.ItemTemplate getTemplate(int id) {
        return GameData.ITEM_TEMPLATES.get(id);
    }
    ...
    public List<Item.ItemOption> getListOptionItemShop(short id) {
        ...
        GameData.SHOPS.forEach(shop -> shop.tabShops.forEach(tabShop -> tabShop.itemShops.forEach(itemShop -> {
