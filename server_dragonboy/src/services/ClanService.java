...
public class ClanService {
    ...
    public Clan getClanById(int id) throws Exception {
        return getClanById(0, GameData.CLANS.size(), id);
    }
    ...
    public List<Clan> getClans(String name) {
        ...
        if (GameData.CLANS.size() <= 20) {
            for (Clan clan : GameData.CLANS) {
                if (clan.name.contains(name)) {
                    listClan.add(clan);
                }
            }
        } else {
            int n = Util.nextInt(0, GameData.CLANS.size() - 20);
            for (int i = n; i < GameData.CLANS.size(); i++) {
                Clan clan = GameData.CLANS.get(i);
                if (clan.name.contains(name)) {
                    listClan.add(clan);
                }
                if (listClan.size() >= 20) {
    ...
    private void acceptAskJoinClan(Player player, int clanMessageId) {
        ...
        if (clan != null && clan.isLeader(player)) {
            ...
            if (cmg != null && !existInClan) {
                ...
                if (pl == null) {
                    pl = PlayerDAO.loadById(plxinvao);
                }
                if (pl != null) {
                    if (pl.idMark.isHoldBlackBall()) {
