...
import matches.SuperRank.SuperRankBuilder;
...
public class SuperRankDAO {
    ...
    public static void insertData(Player player) {
        ...
        try {
            SqlFetcher.executeUpdate("INSERT INTO `super_rank` (`player_id`, `rank`, `name`, `info`, `last_pk_time`, `last_reward_time`, `ticket`, `win`, `lose`, `history`)" +
                            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    player.id,
    ...
    public static void updateData(Player player) {
        ...
        info.put("def", player.nPoint.def);
        try {
            SqlFetcher.executeUpdate("UPDATE `super_rank` SET `rank` = ?, `name` = ?, `info` = ?," +
