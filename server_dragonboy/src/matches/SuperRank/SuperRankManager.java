...
public class SuperRankManager implements Runnable {
    ...
    private final List<SuperRankSession> list;
                ...
                while (iterator.hasNext()) {
                    ...
                    if (wPl != null && wPl.zone != null && wPl.zone.map.mapId == 113) {
                        if (!SPRCheck(wPl.zone)) {
                            list.add(new SuperRankSession(wPl, wsp.rivalId, wPl.zone));
                            iterator.remove();
                        }
                    } else {
                        iterator.remove();
    ...
    public boolean currentlyCompeting(long playerId) {
        for (int i = list.size() - 1; i >= 0; i--) {
            SuperRankSession spr = list.get(i);
    ...
    public void addSPR(SuperRankSession spr) {
        list.add(spr);
    }

    public void removeSPR(SuperRankSession spr) {
        list.remove(spr);
    }
