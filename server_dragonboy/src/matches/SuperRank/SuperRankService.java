...
public class SuperRankService {
    ...
    public void competing(Player player, long id) {
        ...
        if (pl == null) {
            Service.gI().sendThongBao(player, ConstSuperRank.TEXT_KHONG_TIM_THAY_NGUOI_CHOI);
        }
        if (SuperRankManager.gI().currentlyCompeting(player.id)) {
            ...
            Service.gI().sendThongBao(player, ConstSuperRank.TEXT_THIEU_1_NGOC);
            return;
        }
        ...
        switch (menuType) {
            ...
            case 1 -> {
                if (SuperRankManager.gI().SPRCheck(player.zone)) {
                    ...
                    SuperRankManager.gI().addSPR(new SuperRankSession(player, id, player.zone));
                }
            }
            case 2 -> {
                SuperRankManager.gI().addSPR(new SuperRankSession(player, id, getZone(113)));
            }
        }
    }
...
//        Logger.error("load giai sieu hang. processing time: " + (System.currentTimeMillis() - st) + " milliseconds");
    }


    public Player loadPlayer(long id) {
        Player pl = PlayerDAO.loadById(id);
        if (pl != null) {
            pl.setClothes.setup();
            if (pl.pet != null) {
