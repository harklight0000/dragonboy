...
public class NangCapKichHoatVip {
    ...
    public static void startCombine(Player player) {
        ...
        if (itemHD.template.gender == 3 || itemHD.template.type == 4) {
            itemId = GameData.radaSKHVip[Util.nextInt(0, 5)];
            if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                itemId = GameData.radaSKHVip[6];
            }
        } else {
            itemId = GameData.doSKHVip[itemHD.template.gender][itemHD.template.type][Util.nextInt(0, 5)];
            if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                itemId = GameData.doSKHVip[itemHD.template.gender][itemHD.template.type][6];
            }
        }
