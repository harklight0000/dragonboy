...
public class Pet extends Player {
    ...
    public void fusion2(boolean porata) {
        ...
        if (Util.canDoWithTime(lastTimeUnfusion, TIME_WAIT_AFTER_UNFUSION)) {
            if (porata) {
                master.fusion.typeFusion = ConstPlayer.HOP_THE_PORATA_C2;
            }
            this.status = FUSION;
            ChangeMapService.gI().exitMap(this);
            fusionEffect(master.fusion.typeFusion);
    ...
    public short getHead() {
        ...
        // Kiểu pet cố định như cũ
        if (this.typePet == 1) return 297;
        if (this.typePet == 2) return 508;
        if (this.typePet == 3) return 1627;
        if (this.typePet == 4) return (short) (this.isTransform ? 553 : 1624);

        // Skin slot 5 (chỉ dùng nếu hợp lệ)
        Item skin = (inventory != null && inventory.itemsBody.size() > 5) ? inventory.itemsBody.get(5) : null;
        if (skin != null && skin.isNotNullItem()) {
            boolean has38 = itemHasOption(skin, 38);
            if (!(has38 && !masterFused())) { // không chặn khi: không có 38, hoặc có 38 nhưng master fusion
                int part = skin.template.head;
                if (part != -1) return (short) part;
            }
        }
    ...
    public short getBody() {
        ...
        // Kiểu pet cố định như cũ
        if (this.typePet == 1 && !this.isTransform) return 298;
        if (this.typePet == 2 && !this.isTransform) return 509;
        if (this.typePet == 3 && !this.isTransform) return 1628;
        if (this.typePet == 4)                      return 1628;

        // Skin slot 5
        Item skin = (inventory != null && inventory.itemsBody.size() > 5) ? inventory.itemsBody.get(5) : null;
        if (skin != null && skin.isNotNullItem()) {
            boolean has38 = itemHasOption(skin, 38);
            if (!(has38 && !masterFused())) {
    ...
    public short getLeg() {
        ...
        // Kiểu pet cố định như cũ
        if (this.typePet == 1 && !this.isTransform) return 299;
        if (this.typePet == 2 && !this.isTransform) return 510;
        if (this.typePet == 3 && !this.isTransform) return 1629;
        if (this.typePet == 4)                      return 1629;
    ...
    private boolean itemHasOption(Item item, int optId) {
        ...
        for (Item.ItemOption io : item.itemOptions) {
            if (io != null && io.optionTemplate != null && io.optionTemplate.id == optId) return true;
        }
        return false;
    }
    private boolean masterFused() {
        return this.master != null
                && this.master.fusion != null
