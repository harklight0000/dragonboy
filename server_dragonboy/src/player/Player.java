...
import player.intrinsic.IntrinsicPlayer;
import item.Item;
...
import matches.SuperRank.SuperRankData;
...
import player.skill.PlayerSkill;
...
public class Player implements Runnable {
    ...
    public SuperRankData superRank;
    ...
    public Player() {
        ...
        superRank = new SuperRankData(this);
    ...
    private boolean isFused() {
        return this.fusion != null && this.fusion.typeFusion != ConstPlayer.NON_FUSION;
    }

    // Helper: item có option id = optId?
    private boolean itemHasOption(Item item, int optId) {
    ...
    public short getHead() {
        ...
        if (effectSkill != null && effectSkill.isMonkey)    return (short) ConstPlayer.HEADMONKEY[effectSkill.levelMonkey - 1];
        if (effectSkill != null && effectSkill.isSocola)    return 412;

        final boolean fused = isFused();
        Item skin = (inventory != null && inventory.itemsBody.size() > 5) ? inventory.itemsBody.get(5) : null;

        // Skin opt 38 chỉ được hiển thị khi đang fusion
        if (fused && skin != null && skin.isNotNullItem() && itemHasOption(skin, 38) && skin.template.head != -1) {
            return (short) skin.template.head;
        }

        // Outfit fusion mặc định
        if (fused) {
            if (nPoint != null && nPoint.isGogeta_SSJ_4)                           return 1578;
            if (nPoint != null && nPoint.isGogeta_SSJ_Blue)                        return 2100;
            if (fusion.typeFusion == ConstPlayer.LUONG_LONG_NHAT_THE)              return idOutfitFusion[this.gender == ConstPlayer.NAMEC ? 2 : 0][0];
            if (fusion.typeFusion == ConstPlayer.HOP_THE_PORATA)                   return idOutfitFusion[this.gender == ConstPlayer.NAMEC ? 2 : 1][0];
            if (fusion.typeFusion == ConstPlayer.HOP_THE_PORATA_C2)                return idOutfitFusion[3 + this.gender][0];
        }

        // ẩn skin khi không bị chặn bởi opt 38 khi chưa fusion
        if (skin != null && skin.isNotNullItem() && !(itemHasOption(skin, 38) && !fused)) {
            int headId = skin.template.head;
    ...
    public short getBody() {
        ...
        if (effectSkill != null && effectSkill.isMonkey)    return 193;
        if (effectSkill != null && effectSkill.isSocola)    return 413;

        // God outfit khi phụ hộ map Ma Bư và KHÔNG fusion
        if (isPhuHoMapMabu && !isFused()) {
        ...
        // Skin opt 38 chỉ hiển thị khi fusion
        if (fused && skin != null && skin.isNotNullItem() && itemHasOption(skin, 38) && skin.template.body != -1) {
            return (short) skin.template.body;
        }

        // Outfit fusion mặc định
        if (fused) {
            if (nPoint != null && nPoint.isGogeta_SSJ_4)                           return 1581;
            if (nPoint != null && nPoint.isGogeta_SSJ_Blue)                        return 2101;
            if (fusion.typeFusion == ConstPlayer.LUONG_LONG_NHAT_THE)              return idOutfitFusion[this.gender == ConstPlayer.NAMEC ? 2 : 0][1];
            if (fusion.typeFusion == ConstPlayer.HOP_THE_PORATA)                   return idOutfitFusion[this.gender == ConstPlayer.NAMEC ? 2 : 1][1];
            if (fusion.typeFusion == ConstPlayer.HOP_THE_PORATA_C2)                  return idOutfitFusion[3 + this.gender][1];
        }

        // Skin thường: dùng nếu không bị chặn bởi opt 38 khi chưa fusion
        if (skin != null && skin.isNotNullItem() && !(itemHasOption(skin, 38) && !fused)) {
            int body = skin.template.body;
            if (body != -1) return (short) body;
        }

        // Fallback theo áo (slot 0)
    ...
    public short getLeg() {
        ...
        if (effectSkill != null && effectSkill.isMonkey)    return 194;
        if (effectSkill != null && effectSkill.isSocola)    return 414;
        ...
        // Skin opt 38 chỉ hiển thị khi fusion
        if (fused && skin != null && skin.isNotNullItem() && itemHasOption(skin, 38) && skin.template.leg != -1) {
        ...
        if (fused) {
            if (nPoint != null && nPoint.isGogeta_SSJ_4)                           return 1582;
            if (nPoint != null && nPoint.isGogeta_SSJ_Blue)                        return 2102;
            if (fusion.typeFusion == ConstPlayer.LUONG_LONG_NHAT_THE)              return idOutfitFusion[this.gender == ConstPlayer.NAMEC ? 2 : 0][2];
            if (fusion.typeFusion == ConstPlayer.HOP_THE_PORATA)                   return idOutfitFusion[this.gender == ConstPlayer.NAMEC ? 2 : 1][2];
            if (fusion.typeFusion == ConstPlayer.HOP_THE_PORATA_C2)                  return idOutfitFusion[3 + this.gender][2];
        }

        // Skin thường: dùng nếu không bị chặn bởi opt 38 khi chưa fusion
        if (skin != null && skin.isNotNullItem() && !(itemHasOption(skin, 38) && !fused)) {
            int leg = skin.template.leg;
            if (leg != -1) return (short) leg;
        }

        // Fallback theo quần (slot 1)
    ...
    public short getMount() {
        ...
        if (item.template.type == 24 || item.template.type == 23) {
            ...
            if (item.template.id < 500) {
                ...
                return (short) SessionService.MAP_MOUNT_NUM.get(item.template.id);
            }
        }
    }
        ...
        if (!this.isDie()) {
            ...
            if (plAtt != null && !isMobAttack && plAtt.playerSkill.skillSelect != null) {
                switch (plAtt.playerSkill.skillSelect.template.id) {
                    case Skill.KAMEJOKO, Skill.MASENKO, Skill.ANTOMIC, Skill.DRAGON, Skill.DEMON, Skill.GALICK,
                         ...
                         Skill.DICH_CHUYEN_TUC_THOI -> tlNeDon -= plAtt.nPoint.tlchinhxac;
                    default -> tlNeDon = 0;
                }
            ...
            if (!piercing && plAtt != null && plAtt.playerSkill.skillSelect != null) {
                switch (plAtt.playerSkill.skillSelect.template.id) {
                    case Skill.KAMEJOKO, Skill.MASENKO, Skill.ANTOMIC, Skill.DRAGON, Skill.DEMON, Skill.GALICK,
                         Skill.LIEN_HOAN, Skill.KAIOKEN, Skill.QUA_CAU_KENH_KHI, Skill.MAKANKOSAPPO,
                         Skill.DICH_CHUYEN_TUC_THOI -> isUseGX = true;
                }
            }
            ...
            //giam dame khi mp < 20%
            if (this.nPoint.lowMpDefBoost
                    && this.nPoint.getCurrPercentMP() < 20) {
                damage = damage - (damage * this.nPoint.lowMpDefPercent) / 100L;
                if (damage < 1) damage = 1; // tránh về 0/âm
            }
...
//                        System.out.println("[DEBUG] Hiệu ứng lan : " + targetPlayer.name);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
