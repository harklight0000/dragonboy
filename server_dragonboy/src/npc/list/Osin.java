...
public class Osin extends Npc {
    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            ...
            switch (this.mapId) {
                case 50 -> this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ta có thể giúp gì cho ngươi ?",
                        "Đến\nKaio", "Đến\nhành tinh\nBill", "Từ chối");
                ...
                case 50 -> {
                    if (player.idMark.isBaseMenu()) {
                        switch (select) {
                            ...
                            case 1 -> ChangeMapService.gI().changeMap(player, 154, -1, 200, 312);
                        }
                    }
                }
                case 154 -> {
                    if (player.idMark.isBaseMenu()) {
                        switch (select) {
                            case 0 -> ChangeMapService.gI().changeMap(player, 155, -1, 111, 792);
                        }
                    }
                }
                case 155 -> {
