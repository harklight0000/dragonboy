        ...
        if (canOpenNpc(player)) {
            if (this.mapId == 48) {
                switch (player.levelLuyenTap) {
                    case 4 -> this.createOtherMenu(player, ConstNpc.BASE_MENU,
            ...
            if (this.mapId == 48) {
                if (player.idMark.isBaseMenu()) {
                    switch (select) {
                        ...
                        case 3 -> this.createOtherMenu(player, ConstNpc.MENU_DI_CHUYEN,
                                "Ta sẽ đưa con đi",
                                "Về\nthần điện", "Thánh địa\nKaio", "Con\nđường\nrắn độc", "Từ chối");
                    }
                    ...
                    switch (player.levelLuyenTap) {
                        case 5 -> TrainingService.gI().callBoss(player, BossID.THAN_VU_TRU, false);
                        default -> TrainingService.gI().callBoss(player, BossID.KHI_BUBBLES, false);
                    }
                } else if (player.idMark.getIndexMenu() == 2003) {
                    switch (player.levelLuyenTap) {
                        case 4 -> TrainingService.gI().callBoss(player, BossID.KHI_BUBBLES, true);
                        case 5 -> TrainingService.gI().callBoss(player, BossID.THAN_VU_TRU, true);
                        default -> TrainingService.gI().callBoss(player, BossID.THAN_VU_TRU, false);
                    }
