...
import static services.shenron.SummonDragon.*;
...
public class NpcFactory {
    ...
    public static Npc createNPC(int mapId, int status, int cx, int cy, int tempId) {
        int avatar = GameData.NPC_TEMPLATES.get(tempId).avatar;
        try {
            return switch (tempId) {
                ...
                case ConstNpc.TORIBOT -> new ToriBot(mapId, status, cx, cy, tempId, avatar);
                ...
                case ConstNpc.NPC_MAP_164 -> new Myuu(mapId, status, cx, cy, tempId, avatar);
                    ...
                    case 671 -> {
                        switch (select) {
                            case 0 -> {
                                ...
                                NpcService.gI().createTutorial(player, NpcService.gI().getAvatar(ConstNpc.SU_PHU(player)), "Con đã học thành công, hãy đợi");
                                break;
                            }
                            case 1 -> {
