...
public class SessionService {
    ...
    public static String LINK_IP_PORT = Config.gI().getDB_NAME() + ":" + Config.gI().getSV_IP() + ":" + Config.gI().getSV_PORT() + ":0";
    ...
    public static void updateMap(MySession session) {
        ...
        try {
            ...
            msg.writer().writeByte(vsMap);
            msg.writer().writeByte(GameData.MAP_TEMPLATES.length);
            for (MapTemplate temp : GameData.MAP_TEMPLATES) {
                msg.writer().writeUTF(temp.name);
            }
            msg.writer().writeByte(GameData.NPC_TEMPLATES.size());
            for (NpcTemplate temp : GameData.NPC_TEMPLATES) {
                msg.writer().writeUTF(temp.name);
            ...
            msg.writer().writeByte(GameData.MOB_TEMPLATES.size());
            for (MobTemplate temp : GameData.MOB_TEMPLATES) {
    ...
    public static void updateSkill(MySession session) {
        ...
        try {
            ...
            msg.writer().writeByte(GameData.NCLASS.size());
            for (NClass nClass : GameData.NCLASS) {
    ...
    public static void sendBgItemVersion(MySession session) {
        ...
        try {
            msg = new Message(-93);
            msg.writer().writeShort(GameData.BG_ITEMS.size());
            for (BgItem bgItem : GameData.BG_ITEMS) {
    ...
    public static void sendHeadAvatar(Message msg) {
        try {
            msg.writer().writeShort(GameData.HEAD_AVATARS.size());
            for (HeadAvatar ha : GameData.HEAD_AVATARS) {
    ...
    public static void sendImageByName(MySession session, String imgName) {
        ...
        try {
            ...
            msg.writer().writeByte(GameData.getNFrameImageByName(imgName));
