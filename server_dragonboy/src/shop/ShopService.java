...
public class ShopService {
    ...
    private void learnKyNang(Player pl, ItemShop is) {
        ...
        NpcService.gI().createMenuConMeo(pl, 671, NpcService.gI().getAvatar(ConstNpc.SU_PHU(pl)), text, menus);
    }
    ...
    public void showConfirmSellItem(Player pl, int where, int index) {
        ...
        if (item != null && item.isNotNullItem()) {
            ...
            if (item.template.id == 457) {
                if (quantity >= 1) {
                    Input.gI().createFormSellThoiVang(pl);
                    return;
                }
                quantity = 1;
            } else {
    ...
    public void sellThoiVang(Player pl, int quantity){
        long cost = (long) quantity * 500000000;
        Item ThoiVang = InventoryService.gI().findItemBag(pl, 457);
        if (ThoiVang == null){
            Service.gI().sendThongBao(pl, "Không thể thực hiện");
            return;
        }
        if (ThoiVang.quantity < quantity) {
            Service.gI().sendThongBao(pl, "Bạn chỉ có " + ThoiVang.quantity + " thỏi vàng");
        } else {
            if (pl.inventory.gold + cost > Inventory.LIMIT_GOLD) {
                int slban = (int) ((Inventory.LIMIT_GOLD - pl.inventory.gold) / 500000000);
                if (slban < 1) {
                    Service.gI().sendThongBao(pl, "Vàng sau khi bán vượt quá giới hạn");
                } else if (slban < 2) {
                    Service.gI().sendThongBao(pl, "Bạn chỉ có thể bán 1 Thỏi vàng");
                } else {
                    Service.gI().sendThongBao(pl, "Bạn chỉ có thể bán tối đa " + slban + "thỏi vàng");
                }
            } else {
                InventoryService.gI().subQuantityItemsBag(pl, ThoiVang, quantity);
                InventoryService.gI().sendItemBags(pl);
                pl.inventory.gold += cost;
                Service.gI().sendMoney(pl);
                Service.gI().sendThongBao(pl, "Đã bán " + quantity + " Thỏi vàng thu được " + Util.numberToMoney(cost) + " vàng");
            }
        }
    }
