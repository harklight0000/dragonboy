...
public class Boss extends Player implements IBoss {
    ...
    public void update() {
        ...
        switch (this.bossStatus) {
            case CHAT_S, AFK, ACTIVE -> this.autoLeaveMap();
        }
        switch (this.bossStatus) {
            case REST -> this.rest();
            case RESPAWN -> {
    ...
    protected void notifyJoinMap() {
        if (canSendNotify()) {
//            Logger.infoln("BOSS " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName);
    ...
    public void die(Player plKill) {
        if (plKill != null) {
            reward(plKill);
            NLogger.logInformation(plKill.name + ": đã tiêu diệt được " + this.name + " mọi người đều ngưỡng mộ.");
